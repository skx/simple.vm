...
(setq svm-keywords
 '(("^\s*add\\|^\s*sub\\|^\s*mul\\|^\s*div\\|^\s*inc\\|^\s*dec\\|^\s*system\\|^\s*concat\\|^\s*string2int\\|^\s*int2string\\|^\s*cmp\\|^\s*load\\|^\s*print\\|^\s*store\\|^\s*nop\\|^\s*exit" . font-lock-function-name-face)
